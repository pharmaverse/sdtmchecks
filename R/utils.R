...
report_to_xlsx = function(res, outfile) {
  ...
  openxlsx::addWorksheet(wb, "Summary results")
  
  openxlsx::setColWidths(wb, "Summary results", cols=1, widths=30)
  openxlsx::setColWidths(wb, "Summary results", cols=2, widths=65)
  openxlsx::setColWidths(wb, "Summary results", cols=3, widths=20)
  openxlsx::setColWidths(wb, "Summary results", cols=4, widths=35)
  openxlsx::setColWidths(wb, "Summary results", cols=5, widths=25)
  
  openxlsx::addFilter(wb, "Summary results", cols=1:ncol(summary_data), rows=1 )
  
  # write summary data on the 1st page of XLSX file
  openxlsx::writeData(wb, "Summary results", as.data.frame(summary_data), startRow = 1, startCol = 1, headerStyle=createStyle(textDecoration = "bold"))
  
  # Highlight the rows with problematic queries (i.e., have non-missing comments at column D)
  redStyle <- openxlsx::createStyle(fontColour = "#9C0006", bgFill = "#FFC7CE")
  orangeStyle <- openxlsx::createStyle(fontColour = "#000000", bgFill = "#fac966")
  boldnickname <- openxlsx::createStyle(textDecoration = "bold")
  
  openxlsx::conditionalFormatting(wb, "Summary results", cols=1:4 , rows=1:nrow(summary_data)+1, rule='$D2!=" "', style=redStyle)
  openxlsx::conditionalFormatting(wb, "Summary results", cols=2:4 , rows=1:nrow(summary_data)+1, rule='$C2>0', style=orangeStyle)
  openxlsx::conditionalFormatting(wb, "Summary results", cols=1 , rows=1:nrow(summary_data)+1, rule='$C2>0', style=orangeStyle)
  openxlsx::conditionalFormatting(wb, "Summary results", cols=5 , rows=2, rule='$E2!=""', style=boldnickname)
  ...
  #loop through the data checks results and write them into separated sheet in xlsx file.
  for (i in 1:length(res)){
    
    # do not create xls sheet for data checks with 0 results
    if(res[[i]]$nrec != 0) {
      
      openxlsx::addWorksheet(wb, res[[i]]$xls_title)
      ...
      openxlsx::writeData(wb, res[[i]]$xls_title, as.data.frame(res[[i]]$data), startRow = 1, startCol = 1)
      ...
      openxlsx::writeData(wb, sheet="Summary results", startRow=i+1, startCol=1,
                x=res[[i]]$xls_title )
      ...
      openxlsx::writeComment(wb, res[[i]]$xls_title, col=1, row=1,
                             comment=createComment(
                     unlist(summary_data_0[i,"pdf_subtitle"]),
                     author = "sdtmchecks",
                     visible = FALSE,
