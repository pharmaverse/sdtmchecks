...
**Q: How is the designation of High, Medium, Low priority assigned in the sdtmchecksmeta package?**
...
**A:** The package is not available on CRAN and needs to be installed from other repos. The syntax `install.packages("sdtmchecks", repos = "https://pharmaverse.r-universe.dev")` or  `devtools::install_github("pharmaverse/sdtmchecks")` can be used.
...
**A:** In general, non-standard data are not covered by these checks. In oncology Response (RS) domain data, the emphasis is on investigator-based results rather than independent review facility or blinded independent central reader (BICR) data reported by a vendor in light of the former typically being more standardized. 

<br>


**Q: Does SDTM IG version information need to be entered in order to use the package?**

**A:** No, the data checks do not require version information as initial input. 

<br>

**Q: What versions of SDTM are covered in the sdtmchecks package?**

**A:** The existing data checks are intended to broadly flag data issues. Some workarounds have been incorporated in data checks that include the EXOCCUR variable, which is not in SDTM IG 1.4 but was in Exposure (EX) domains based on earlier IG versions. Most checks run regardless of the study build date and SDTM version. Please feel free to share recommendations on making the checks more robust via logging an [issue](https://github.com/pharmaverse/sdtmchecks/issues).

We plan to augment the package documentation in coming weeks to include more information on SDTM IG coverage. 

<br>

**Q: Which SDTM Implementation Guide (IG) version is used as the basis of the checks?**

**A:** The data checks are intended to run regardless of SDTM IG version. There are a few checks that only apply to a specific version. For example, "Check for EXDOSE>0 when EXOCCUR is not 'Y'" will return "EX is missing the variable: EXOCCUR" for SDTM IG 3.2+ studies. 
