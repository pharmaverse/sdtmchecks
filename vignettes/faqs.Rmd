...
**Q: Which SDTM domains are referenced in data check functions?**

```{r, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE }
library(sdtmchecks)
sdtm <- sdtmchecksmeta
myvec  <- unlist(strsplit(sdtm$domains,", "))
mydomains <- dplyr::arrange(dplyr::distinct(as.data.frame(myvec)))
domains <- toupper(mydomains$myvec)
```

**A:** The following `r nrow(mydomains)` domains are referenced in existing check functions:

```{r, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
knitr::kable(as.list(domains), col.names = NULL, format = "html")
```
